...
var yadcf = ( function( $ ) {
    ...
    function appendSelectFilter( args ) {
        ...
        var i = 0;
        var $filter_selector;
        var filter_selector_string;
        var data;
        var filter_container_id;
        var column_number;
        var column_data_type;
        var html_data_type;
        var text_data_delimiter;
        var filter_default_label;
        var filter_reset_button_text;
        var options;
        var j;
        var k;
        var data_length;
            ...
            if( $filter_selector.length === 1 ) {
                ...
                if( filter_container_id === undefined ) {
                    $( filter_selector_string ).append( "<select class=\"yadcf-filter\" " +
                            "onchange=\"yadcf.doFilter('" + filter_selector_string + "', this, " + column_number +
                            ")\" onclick='event.cancelBubble = true;event.stopPropagation();'>" + options + "</select>" );
                    ...
                    $( filter_selector_string ).find( ".yadcf-filter" ).after( "<input value=\"" + filter_reset_button_text + "\" type=\"button\" " +
                            "onclick=\"event.cancelBubble = true;event.stopPropagation();yadcf.doFilter('" + filter_selector_string + "', 'clear', " +
                            column_number + "); return false;\" class=\"yadcf-filter-reset-button\">" );
                } else {
                    ...
                    $( "<select class=\"yadcf-filter\" " +
                            "onchange=\"yadcf.doFilter('" + filter_selector_string + "', this, " + column_number +
                            ")\" onclick='event.cancelBubble = true;event.stopPropagation();'>" +
                            options + "</select>" ).appendTo( "#" + filter_container_id );

                    $( "#" + filter_container_id ).find( ".yadcf-filter" ).after( "<input value=\"" +
                            filter_reset_button_text + "\" type=\"button\" " +
                            "onclick=\"event.cancelBubble = true;event.stopPropagation();yadcf.doFilter('" +
                            "#" + filter_container_id + "', 'clear', " + column_number + "); return false;\" class=\"yadcf-filter-reset-button\">" );
                }
            }

...
class Approximate_Search
{
    ...
    {
        if( $text_strlen < 0 ) $text_strlen = strlen( $text );

        if( $max_len < 0 ) $max_len = $text_strlen;
        ...
        if( $n < $m )
        {
            ...
            if( $lev <= $k ) return Array( $start_index + $n - 1 => $lev );
            else return Array();
        }
        ...
        {
            $c = $patt{$i};
            if( isset( $s[$c] ) ) $s[$c] = min( $i, $s[$c] );
            else $s[$c] = $i;
        }

        if( $end_index < $start_index ) return Array();
        ...
        for( $t = $start_index; $t < $end_index; $t++ )
        {
            ...
            for( $i = 1; $i <= $mk; $i++ )
            {
                ...
                if( $in_patt ) for( $j = $e[$i - 1]; ($j < $g && $j <= $mk ); $j++ ) if( $patt{$i + $j - 1} == $c ) $g = $j;
            ...
            if( $d[$mk] <= $k )
            {
                ...
                if( !isset( $matches[$i] ) || $err < $matches[$i] ) $matches[$i] = $err;
            }
        }
    ...
    function test_short_search()
    {
        ...
        assert( $this->search_short( $test_patt, 4, $test_text ) == Array( 27 => 2,
            60 => 1, 94 => 0 ) );
        assert( $this->search_short( $test_patt, 2, $test_text ) == Array( 27 => 2,
            60 => 1, 94 => 0 ) );
        assert( $this->search_short( $test_patt, 1, $test_text ) == Array( 60 => 1,
            94 => 0 ) );
        assert( $this->search_short( $test_patt, 0, $test_text ) == Array( 94 => 0 ) );
        assert( $this->search_short( "bjorling", 2, $test_text, 19, 7 ) == Array() );
        assert( $this->search_short( "bjorling", 2, $test_text, 19, 8 ) == Array(
            26 => 2 ) );
    ...
    function Approximate_Search( $pattern, $max_error )
    {
        ...
        for( $i = 0; $i < $this->patt_len; $i+=$intpartlen )
        {
            ...
            else $this->parts[] = substr( $this->patt, $i, $intpartlen );
        }
        ...
        foreach( $this->parts as $p ) $this->max_part_len = max( $this->max_part_len, strlen( $p ) );
    ...
    function search( $text )
    {
        ...
        while( list($i, $p) = each( $part_map ) )
        {
            ...
            {
                if( $group_len >= ($this->n_parts - $this->max_err) ) $transf[] = Array(
                        $transf_text, $transf_pos );
        ...
        if( strlen( $transf_text ) >= ($this->n_parts - $this->max_err) ) $transf[] = Array(
                $transf_text, $transf_pos );

        unset( $transf_text, $transf_pos );

        if( current( $transf ) === False ) return Array();
...
//                    print_r( $this->search_short( &$this->patt, $this->max_err, &$text, $start, $end-$start ));
        ...
        if( current( $part_positions ) === False ) return Array();

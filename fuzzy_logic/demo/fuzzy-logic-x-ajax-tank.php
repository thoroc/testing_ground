...
class Fuzzify extends Member
{
    ...
    {
        foreach( $this->members[$idx] as $idx => $member ) if( $value == $member->FName ) return $idx;
        return FALSE;
    }

    public function getMemberByName( $idx, $name = NULL )
    {
        foreach( $this->members[$idx] as $idx => $member ) if( $name == $member->FName ) return $member;
        return FALSE;
    }
}
// end class fuzzify
...
class Rules extends Fuzzify
{
    ...
    {
        if( preg_match( "/\((([^()]*|(?R))*)\)/", $string, $matches ) ) return $matches[1];
    }
...
class Fuzzy_Logic extends Rules
{
    ...
    public function calcFuzzy()
    {
        ...
        foreach( $this->getOutputNames() as $outname )
        {
            ...
            if( $sumb == 0 ) $result[$outname] = 0;
            else $result[$outname] = $suma / $sumb;
        }
        return $result;
    }
}
...
for( $tank = 0; $tank < 7; $tank++ )
{
    ...
    if( $key_input_ref >= 0 ) $Vinp_max = $tankInpMax[$tank] * $tankInpDT[$tank] * ( int ) (($tankLevel[$tank] <= $tankMaxWorkLevel[$tank]) and ( $tankLevel[$key_input_ref] > $tankMinWorkLevel[$key_input_ref]));
    else $Vinp_max = $InputMaxValve * $Tinp_stream * ( int ) ($tankLevel[$tank] < $tankMaxWorkLevel[$tank]);

    list($tankValve[$tank], $WaterInput) = getValve( $tankFuzzy[$tank], $tankValve[$tank], $Vinp_max, 0, 1 );

    $key_output_ref = $tankOutputRefKey[$tank];

    if( $key_output_ref >= 0 ) $VOut_max = $tankOutMax[$tank] * $tankOutDT[$tank] * ( int ) ($tankLevel[$key_output_ref] > $tankMinWorkLevel[$key_output_ref]);
    else //-1
